<template>
  <div>
    <div class="q-mb-md" v-for="(job,index) in jobs" :id="'card-row-'+job.id" :key="job.id">
      <q-card  class="my-card shado-1" dense flat>
        <q-item>
        ...
        <q-separator />
        <p  class="q-pt-md q-pl-md text-grey-6">{{ job.description }}</p>
        <q-separator />
        <q-card-actions class="q-pb-md q-pt-md q-pl-md">
          <q-btn color="orange"   padding="5px 30px" label="Accept" @click="confirmAction('accepted', job.id,index)"  text-color="white"/>
          <q-btn color="grey" padding="5px 30px" @click="confirmAction('declined', job.id,index)" label="Decline"   class="text-grey-6"/>
          <q-btn :label="job.lead_price"  flat />
        </q-card-actions>
      </q-card>
    </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    getLeads: function () {
      this.showDefault()
      api.get('/job?status=new').then((response) => {
        this.jobs = response.data.data
      }).catch(() => {
        this.triggerNegative()
      })
